#LOCATE_PYTHON_MODULE(<module> [PATHS <path1> ... <patN>] [REQUIRED])
#
...
function(LOCATE_PYTHON_MODULE module)
        ...
        if(NOT PY_${module_upper})
                ...
                # Use modern Python importlib to find the location of the requested module
                execute_process(COMMAND "${PYTHON_EXECUTABLE}" "-c"
                        "import importlib.util; spec = importlib.util.find_spec('${module}'); print(spec.origin if spec and spec.origin else '')"
                        RESULT_VARIABLE _${module}_status
                        OUTPUT_VARIABLE _${module}_location
                        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)

                # If importlib approach fails, try direct import as fallback
                if(_${module}_status OR NOT _${module}_location)
                        execute_process(COMMAND "${PYTHON_EXECUTABLE}" "-c"
                                "import ${module}; import os; print(${module}.__file__ if hasattr(${module}, '__file__') else '')"
                                RESULT_VARIABLE _${module}_status
                                OUTPUT_VARIABLE _${module}_location
                                ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
                        
                        # If we got a .py file, get its directory
                        if(NOT _${module}_status AND _${module}_location)
                                get_filename_component(_${module}_location "${_${module}_location}" DIRECTORY)
                        endif()
                endif()
